...
parse_documentation() {
    documentation="$(grep "^##\s" "$(which "$1")")(no-trim)"
    #documentation=$(echo "$documentation" | $SED_REGEX "s/## ?//" | $SED_REGEX "s/@script.name/$(basename "$1")/g" | sed "s/@#/@/g")
    #documentation=$(echo "$documentation" | $SED_REGEX "s/## ?//" | $SED_REGEX "s/@script.name/$(basename "$1")/g" | sed "s/@#/@/g")
    documentation=$(echo "$documentation" | $SED_REGEX "s/## ?//" | sed "s/@#/@/g")
    documentation=${documentation%(no-trim)}
}

parse_specifications() {
    specifications="$(grep "^###\s" "$(which "$1")")(no-trim)"
    specifications=$(echo "$specifications" | $SED_REGEX "s/### ?//" | $SED_REGEX "s/@script.name/$(basename "$1")/g" | sed "s/@#/@/g")
    specifications=${specifications%(no-trim)}
}
...
parse_options() {
    ...
    # extract script name and shift parameters
    scriptname=$1
    shift
    ...
    while getopts ":${short_options}-:" option; do
        ...
        # Help option
        if [[ -n "$help" ]]; then
            [[ -z "$documentation" ]] && parse_documentation $scriptname
            echo "$documentation"
            return
        fi
    done
